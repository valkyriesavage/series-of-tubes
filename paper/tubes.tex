...
To allow overhangs, 3D printers lay down support material that must be later removed from the model.  This removal process can be time-intensive and challenging.  The physical fabrication process used by different machines gives rise to differing strategies for avoiding support material or easing its removal.  We will discuss techniques for fused deposition modeling (FDM) machines, such as the Makerbot\footnote{\url{http://makerbot.com}}, multi-jetting machines such as an Objet\footnote{\url{http://stratasys.com}}, and powder-based machines such as the ZCorp\footnote{\url{http://3dsystems.com}}.  Other varities of machines, e.g., stereolithography machines such as the Form 1\footnote{\url{http://formlabs.com}}, require similar techniques to these.
...
\subsection{Powder-based}
Powder-based machines have a bed of powder which is selectively fused together using glue droplets.  As each layer is completed, the bed moves down and another layer of powder is added on top.  The unfused powder functions as the support structure for the fused powder: this makes the support trivial to remove post-print, as the loose grains will simply fall out of pipes and cavities in the model.  These machines unfortunately cannot fabricate flexible or transparent materials (yet), however they can print in full-color.  We did not have access to a powder-based printer, though in future work we would like to experiment with one.

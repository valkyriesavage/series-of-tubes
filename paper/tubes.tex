...
The wire is modelled as a \emph{discrete rod}, ie a 3D poly-line with a bending 
...
a boolean union with the initial shape-minus-tube result.
A straightforward extension would be to add or subtract additional elements at
the tube endpoints to make them non-round.
 \ryan{Figure 8 is not necessary. This is a known problem with any voxelization method. Nobody is going to care that we took shortcuts in our implementation like repeatedly voxelizing and meshing instead of updating a single initial voxelization.}

...
\bjoern{Hits section mentions a* three times, ech time with a little more detail than before. An probably cut this to two times as a general strategy: once in the overview, then a second time in detail.}
...
\bjoern{Replace with a single sentence: combining pipe routing with other existing mesh modification operations like extrusions enables users to create custom ...}
...
\bjoern{Replace specific with more general category: pipes where interior path layout matters}
...
\tovi{Still too long....}
\bjoern{Need this explanation above so readers don't wonder what Euler tours are

Agree with Tovi that this can be condensed}

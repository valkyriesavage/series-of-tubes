...
To create pipes in which the location and shape of exterior connection points matters, we first allow makers to select these points on the mesh's surface. \bjoern{feels a bit redundant with the last paragraph in the prior section.} 
...
In addition, distance from the surface is not taken into account in A* search and pipes may thus be routed along surface contours, which may cause leakage.
...
In the case of multiple initial paths, we solve for each smoothed path sequentially.\bjoern{Why would we ever end up with multiple paths here? It thought we processed each path by running it through the (A*-PBD-Cutout) pipeline. So this step should only ever see one path, no?}
After solving for a path, we add it as a constraint to the system, generating
a penalty force that pushes additional paths away from it. \bjoern{are there any guarantees that two paths are more than $pipe1{radius}+pipe2{radius}$ away from each other? if not, we can't really claim that we always avoid intersections - it just usually happens.}
...
The simulation takes about a second for most of our examples.

Pipes are routed and cut out of the mesh serially. This prevents pipe intersections: once a pipe has been cut out of the mesh, it cannot be part of valid routes in any future pipe routing operation.  However, we do not currently track all pipes for global optimization: we greedily select the best routing per pipe.
Our voxel grid currently uses a resolution of 128x128x128 voxels, which enables interactive operations, but this parameter can be adjusted for different model requirements.
...
allows different start and end radii, which are linearly interpolated along the path. \bjoern{why is this useful?}

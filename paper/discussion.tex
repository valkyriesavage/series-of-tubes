...
\section{Discussion}
%\valkyrie{there are a lot of things in here that we could focus on.  input/output/identity interactions mediated by tubes, the creation of antennae and other custom electronics...}
%
%\valkyrie{which figures/discussions do we think are important?}
...
\caption{By attaching a servo to a gate structure, it is possible to direct fluid through that pipe.  The gate not only has binary positions (a) and (b), but can direct fractional flow, as well (c).  This could be used to create logic gates, as in (d).  These could be used, for example, in science exhibits. \bjoern{If we use this figure, do the top row only, not the bottom (too big).}}
\label{fig:direct}
\end{figure}
...
\subsection{Inputs}
%In addition to the described ways for sensing touch and object presence, additional opportunities also exist for leveraging pipes to sense pressure, grasp, flexing, and tapping. 
\bjoern{most of the beginning of this section described techniques we had already implemented or described earlier in the paper. They  referenced earlier figures and earlier citations. I commented them out as I think this section is about additional techniques nto already described. For example, we referenced slyper  in the design space. this feels empty now though. I'm not quite sure what the right thing to do is.}
% and manipulation of traditional electronic components.
...
%\bjoern{we alsready covered touch in the paper... think this section should be about different ways. \emph{Touch} sensing can be enabled through pipes filled with conductive media.  Traditional wires (threadable) may be used, but conductive paint (liquid) may be more flexible.  Using conductive paint and SFCS \cite{Sato-touche}, an interior star pipe topology can enable single-wire touch- and grasp-sensing at any set of points on a printed object's surface, like our toys (see Figure \cite{fig:toys}).  Simple capacitance measurements are possible using the same media, and custom-designed sensors like those created in Savage, et al.,'s Midas system are also possible \cite{Savage-midas}.  Using a technique like Fiberio \cite{Holz-fiberio}, users could even be identified on touches. \bjoern{Fiberio requires fiber optic plate - not sure it applies to pipes.}
...
Both identification by recall and intentional encoding are possible.  As seen in Touch \& Activate \cite{Ono-touchandactivate}, different objects have different acoustic signatures.  Additionally, two objects that are visually identical but which have fully enclosed (or other types) of pipes on their interior can be distinguished acoustically.  Thus we can recall an object's identity once its acoustic signature has been recorded.  Similarly, the Touch\'{e} system \cite{Sato-touche} relies on distinguishing capacitive signatures of objects, both in static states and as their boundary conditions are changed by human interaction.  Weight is a simple metric of object identity, but weights are less distinctive than acoustic or capacitive signatures, as they are represented by a single number. \bjoern{second half of this paragraph meanders. focus on the key story.}
